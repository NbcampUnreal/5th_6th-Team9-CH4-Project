...
void UUIManagerSubsystem::SetUIState(EGameUIState NewState)
{
    ...
    if (GetWorld() && GetWorld()->GetNetMode() != NM_DedicatedServer)
    {
        ...
        ULocalPlayer* LocalPlayer = GetWorld() ? GetWorld()->GetFirstLocalPlayerFromController() : nullptr; //로컬이 아니라 서버로 해야함
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("Success Local PlayerController NotReady"));      //무한대로 나오는 현상

            GetWorld()->GetTimerManager().SetTimer(DelayHandle, [this, NewState]()        //SetTimerForNextTick 대신 1초 정도로 해놓기
                {
                    SetUIState(NewState);
                }, 1.0f, false);
            return;
